...
def laser_acc(bodies, n, laser_power, burn_time):
    for body in bodies:
        if body.name == "asteroid":
            ...
            laser_acc = math.sqrt(1.596e-3*(laser_power-1358.41))   # in N
            ...
            if n < number_of_time_intervals:   # the less than equal sign is fixed
                ...
                acc_x = (math.sin(theta)*laser_acc)/body.mass
                acc_y = (-math.cos(theta)*laser_acc)/body.mass
                return acc_x, acc_y
...
ln, = plt.plot([], [], "ro", markersize = 6, animated=True, zorder=10)      # 'ro' = red circle 'co' = cyan circle ...
ln1, = plt.plot([],[], "co", markersize = 0.05, animated = True, zorder=0)   # markersize = how big the circle is
earth, = plt.plot([],[], "go", markersize = 6, animated=True, zorder=5)
ttl = ax.text(0, 0.01, '', transform = ax.transAxes,fontsize = 10, color="white")
sun, = plt.plot([],[], "yo", markersize = 10, animated=True, zorder=0)
# ln is the current position, ln1 is the past position


def init():
    ax.set_xlim(-2e11, 2e11)     # these 2 define the scale of the graph
...
def update(frame):
    ...
    for body in bodies:
        if body.name == "asteroid":
            ...
            xdata3.append(body.location.x)
            ydata3.append(body.location.y)

    ln.set_data(xdata, ydata)
    earth.set_data(xdata2, ydata2)
    sun.set_data(xdata3, ydata3)
    for body in bodies:
    ...
    def runtime(frame):
        frame *= 4
        num_years = frame // 8760
        frame -= num_years * 8760
        num_days = frame // 24
        frame -= num_days * 24
        num_hours = frame

        if num_years == 1:
            print_years = str(num_years) + " year, "
        else:
            print_years = str(num_years) + " years, "
        if num_days == 1:
            print_days = str(num_days) + " day, "
        else:
            print_days = str(num_days) + " days, "
        if num_hours == 1:
            print_hours = str(num_hours) + " hour."
        else:
            print_hours = str(num_hours) + " hours."

        print("Total time elapsed: ", print_years, print_days, print_hours)
    if a_e_distance < 1.5e9:
        ...
        runtime(frame)
        sys.exit(0)
    time_years = (frame * 4) // 8760
    time_days = ((frame * 4) - (time_years * 8760)) // 24
    ttl.set_text(str("Time elapsed: "+ str(time_years) + " years, " + str(time_days) + " days" + "   Burn Time: " + str(int(burn_time)//3600)+' hours'))

    return ln, ln1, earth, ttl, sun

ani = FuncAnimation(fig, update, interval = 0.01, init_func=init, blit=True)
